...
class HospitalOverviewScene(QGraphicsScene):
    ...
    def _add_segment_shape(self, spec: SegmentSpec):
        ...
        # Narrow centerline + soft glow keeps the overlay inside the corridor.
        glow_color = QColor(84, 166, 255, 70) if spec.active else QColor(107, 114, 128, 48)
        core_color = QColor(PALETTE["accent"] if spec.active else PALETTE["route_outline"])
        halo_pen = QPen(glow_color, 22 if spec.active else 14)
        ...
        core_pen = QPen(core_color, 10 if spec.active else 6,
...
class ResponsiveMapView(QGraphicsView):
    ...
    def wheelEvent(self, event):
        self._apply_zoom_step(1 if event.angleDelta().y() > 0 else -1)
        event.accept()
    ...
    def fit_scene(self, rect: Optional[QRectF] = None):
        self.resetTransform()
        self.fitInView(rect or self.sceneRect(), Qt.AspectRatioMode.KeepAspectRatio)
        self._zoom_steps = 0
        self._manual_zoom = False
        self.setDragMode(QGraphicsView.DragMode.NoDrag)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Hospital Asset Tracker — Fullscreen Rollout Demo v7")
        ...
        splitter.addWidget(
            MapCard(
                ...
                "Overlays now follow actual corridor centerlines on the SJH south floorplan",
                self.hospital_view,
        ...
        splitter.addWidget(
            MapCard(
                ...
                "Original working reader segment with cleaner presentation scaling",
                self.pilot_view,
