...
1) Recommended multi-asset format:
   SCAN:LEFT:AA:BB:CC:DD
   SCAN:RIGHT:11:22:33:44

   - The app will place the asset at LEFT or RIGHT.
   - If the UID is unknown, it prompts you to name the asset and choose a symbol.
   - Assets stay at their last known spot until scanned somewhere else.
...
2) Legacy compatibility:
   ...
   - This moves a default wheelchair asset.
...
class AssetRegistrationDialog(QDialog):
    def __init__(self, uid: str, parent=None):
        ...
        self.setStyleSheet(f"""
            ...
            QLabel, QLineEdit, QComboBox {{ font-family: Consolas; font-size: 11px; }}
            QLineEdit, QComboBox {{
        ...
        self.name_edit.setText(f"Asset {uid[-5:].replace(':', '')}")
    ...
    def get_data(self):
        return self.name_edit.text().strip(), self.symbol_combo.currentText()
...
class SerialWorker(QTimer):
    ...
    position_received = pyqtSignal(str)   # legacy mode
    ...
    def _read(self):
        ...
        while "\n" in self._buf:
            ...
            if line.startswith("SCAN:"):
                ...
                if key in ("LEFT", "CENTER", "RIGHT", "UNKNOWN"):
                    self.position_received.emit(key)
...
class SidePanel(QWidget):
    ...
    def _build_ui(self):
        ...
        hint = QLabel("Recommended serial format:\nSCAN:LEFT:AA:BB:CC:DD")
    ...
    def update_asset(self, uid: str, name: str, symbol: str, pos: str):
        ...
        for asset_uid, meta in self.asset_meta.items():
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Hospital Asset Tracker — RFID v2")
        ...
        self.statusBar().showMessage(
            "  Connect Arduino, then scan tags. New UIDs will prompt for name + symbol."
        )

        # default legacy asset
        self.scene.ensure_asset("LEGACY_WHEELCHAIR", "Wheelchair", "♿")
        self.scene.move_asset("LEGACY_WHEELCHAIR", "CENTER")
        self.panel.update_asset("LEGACY_WHEELCHAIR", "Wheelchair", "♿", "CENTER")
    ...
    def _connect_serial(self, port: str):
        ...
        self._serial_worker.position_received.connect(self._handle_legacy_position)
    ...
    def _on_raw_message(self, line: str):
        if line.startswith("SCAN:") or line.startswith("POS:"):
            return
        self.panel.set_event_text(line[:60])

    def _register_asset_if_needed(self, uid: str):
        if self.scene.asset_exists(uid):
            return

        dialog = AssetRegistrationDialog(uid, self)
        if dialog.exec():
            name, symbol = dialog.get_data()
        else:
            name, symbol = f"Asset {len(self.scene.assets)}", "📟"
    ...
    def _handle_scan(self, reader: str, uid: str):
        ...
        self._register_asset_if_needed(uid)
    ...
    def _handle_legacy_position(self, key: str):
        if key == "UNKNOWN":
            self.panel.set_event_text("Legacy mode: signal lost, keeping last known spot")
            self.statusBar().showMessage("  Legacy asset signal lost — keeping last known spot")
            return

        self.scene.move_asset("LEGACY_WHEELCHAIR", key)
        asset = self.scene.get_asset("LEGACY_WHEELCHAIR")
        if asset:
            self.panel.update_asset("LEGACY_WHEELCHAIR", asset.name, asset.symbol, key)
        self.panel.set_event_text(f"Legacy wheelchair moved to {key}")
        self.statusBar().showMessage(f"  Legacy wheelchair position: {key}")
