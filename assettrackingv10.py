...
- The active RFID pilot hallway is aligned to the real upper corridor from the
  attached SJH Level 1 South floorplan.
- Additional rollout hallways are tightened to corridor centerlines.
- Hospital map interaction is more intuitive in fullscreen: wheel scrolls the
  map vertically, Shift+wheel scrolls horizontally, Ctrl+wheel zooms.
- The hospital map defaults to a fit-width inspector view so you can scroll all
  the way from the top to the bottom of the floorplan.
...
class ResponsiveMapView(QGraphicsView):
    def __init__(self, scene, min_size: Tuple[int, int] = (900, 440), default_fit_mode: str = "whole"):
    ...
    def fit_width(self, rect: Optional[QRectF] = None):
        ...
        if target_rect.width() <= 0 or viewport_rect.width() <= 2:
            return
        usable_width = max(10, viewport_rect.width() - 6)
        ...
        self.ensureVisible(target_rect.left(), target_rect.top(), 1, 1, 0, 0)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.pilot_view = ResponsiveMapView(self.pilot_scene, min_size=(1080, 270), default_fit_mode="whole")
    ...
    def _reset_views(self):
        self.hospital_view.reset_to_default_fit(self.hospital_scene.sceneRect())
