...
"""
assettrackingv11.py
Hospital Asset Tracker — fullscreen-optimized hospital rollout demo.

What this version adds:
- The active RFID pilot hallway stays aligned to the real upper corridor from
  the attached SJH Level 1 South floorplan.
- Fullscreen behavior is improved: the main map now uses a true fit-width
  startup view, preserves scrolling better, and avoids awkward resize resets.
- The right-side map area is more responsive, with lighter minimum sizes and a
  better vertical split for presentation/fullscreen use.
- Additional rollout hallways stay tightened to corridor centerlines.
- Asset stacking stays inside corridor widths more cleanly.
...
SEGMENTS: Dict[str, SegmentSpec] = {
    # Active pilot moved onto the real top corridor so the tracked segment sits
    # directly on the upper hallway in the attached floorplan.
    "PILOT": SegmentSpec(
        name="Top Clinic Corridor Pilot",
        path_points=(
            (778, 184),
            (840, 184),
            (904, 184),
    ...
    # Real west-side vertical corridor between the west rooms and the core.
    ...
    # Right-side vertical public spine on the east edge of the crop.
    ...
    # South-east connector that stays centered in the hallway, not inside rooms.
...
STACK_OFFSETS_HORIZONTAL = [QPointF(0, 0), QPointF(0, -7), QPointF(0, 7), QPointF(0, -13), QPointF(0, 13)]
STACK_OFFSETS_VERTICAL = [QPointF(0, 0), QPointF(-7, 0), QPointF(7, 0), QPointF(-13, 0), QPointF(13, 0)]
...
class HospitalAssetMarker(QGraphicsObject):
    ...
    def paint(self, painter: QPainter, option, widget=None):
        ...
        glow = QRadialGradient(0, 0, 18)
        glow.setColorAt(0, QColor(88, 166, 255, 95))
        glow.setColorAt(1, Qt.GlobalColor.transparent)
        painter.setBrush(QBrush(glow))
        painter.setPen(Qt.PenStyle.NoPen)
        ...
        painter.setPen(QPen(QColor(PALETTE["pilot_outline"]), 1.8))
        painter.drawEllipse(-11, -11, 22, 22)

        painter.setPen(QColor("#FFFFFF"))
        painter.setFont(QFont("Segoe UI Emoji", 10))
        painter.drawText(QRectF(-11, -11, 22, 22), Qt.AlignmentFlag.AlignCenter, self.symbol)

        painter.setPen(QColor("#0F1720"))
        painter.setFont(QFont("Consolas", 6, QFont.Weight.Bold))
        painter.drawText(QRectF(-34, 12, 68, 12), Qt.AlignmentFlag.AlignCenter, self.name)

        painter.setPen(QColor("#334155"))
        painter.setFont(QFont("Consolas", 5))
        painter.drawText(QRectF(-34, 22, 68, 10), Qt.AlignmentFlag.AlignCenter, self.checkpoint_key)
...
class HospitalOverviewScene(QGraphicsScene):
    ...
    def _add_segment_shape(self, spec: SegmentSpec):
        ...
        halo_pen = QPen(glow_color, 8 if spec.active else 6)
        ...
        core_pen = QPen(core_color, 3.5 if spec.active else 2.5,
    ...
    def _stack_offset(self, segment_key: str, count: int) -> QPointF:
        spec = SEGMENTS[segment_key]
        offsets = STACK_OFFSETS_VERTICAL if spec.orientation == "vertical" else STACK_OFFSETS_HORIZONTAL
        return offsets[min(count, len(offsets) - 1)]
    ...
    def update_asset(self, uid: str, name: str, symbol: str, segment_key: str, checkpoint_key: str):
        ...
        offset = self._stack_offset(segment_key, same_pos_count)
    ...
    def show_history_trail(self, entries: List[Dict[str, str]]):
        ...
        for i, pt in enumerate(dedup_points, start=1):
            dot = self.addEllipse(pt.x() - 5, pt.y() - 5, 10, 10,
...
class ResponsiveMapView(QGraphicsView):
    def __init__(self, scene, min_size: Tuple[int, int] = (320, 220), default_fit_mode: str = "whole"):
        super().__init__(scene)
        self.default_fit_mode = default_fit_mode
    ...
    def _scroll_amount(self, event, horizontal: bool = False) -> int:
        pixel = event.pixelDelta()
        if not pixel.isNull():
            value = pixel.x() if horizontal else pixel.y()
            if value:
                return value
        angle = event.angleDelta().x() if horizontal else event.angleDelta().y()
        if angle == 0:
            angle = event.angleDelta().y() if horizontal else event.angleDelta().x()
        steps = angle / 120 if angle else 0
        base = 72
        return int(steps * base)

    def wheelEvent(self, event):
        modifiers = event.modifiers()

        if modifiers & Qt.KeyboardModifier.ControlModifier:
            delta = event.angleDelta().y() or event.angleDelta().x() or self._scroll_amount(event)
            self._apply_zoom_step(1 if delta > 0 else -1)
            event.accept()
            return

        if modifiers & Qt.KeyboardModifier.ShiftModifier or abs(event.angleDelta().x()) > abs(event.angleDelta().y()):
            bar = self.horizontalScrollBar()
            amount = self._scroll_amount(event, horizontal=True)
            bar.setValue(bar.value() - amount)
        else:
            bar = self.verticalScrollBar()
            amount = self._scroll_amount(event, horizontal=False)
            bar.setValue(bar.value() - amount)
        event.accept()

    def mouseDoubleClickEvent(self, event):
        self.reset_to_default_fit()
        event.accept()

    def showEvent(self, event):
        super().showEvent(event)
        if not self._did_initial_fit and self.viewport().width() > 40 and self.viewport().height() > 40:
            self._did_initial_fit = True
            QTimer.singleShot(0, self.reset_to_default_fit)
    ...
    def fit_width(self, rect: Optional[QRectF] = None):
        self.resetTransform()
        target_rect = rect or self.sceneRect()
        viewport_rect = self.viewport().rect()
        if target_rect.width() <= 0 or viewport_rect.width() <= 2:
            return
        usable_width = max(10, viewport_rect.width() - 18)
        factor = usable_width / target_rect.width()
        self.scale(factor, factor)
        self._zoom_steps = 0
        self._manual_zoom = False
        self.setDragMode(QGraphicsView.DragMode.NoDrag)
        self.centerOn(target_rect.center().x(), target_rect.top())
        self.horizontalScrollBar().setValue(self.horizontalScrollBar().minimum())
        self.verticalScrollBar().setValue(self.verticalScrollBar().minimum())

    def reset_to_default_fit(self, rect: Optional[QRectF] = None):
        if self.default_fit_mode == "width":
            self.fit_width(rect)
        else:
            self.fit_scene(rect)
...
class MapCard(QWidget):
    ...
    def fit_view(self):
        ...
        self.view.reset_to_default_fit(rect)
...
class SidePanel(QWidget):
    ...
    def _build_ui(self):
        ...
        subtitle = QLabel("Fullscreen-ready layout · simple controls · route + history")
        ...
        footer = QLabel("Wheel = scroll • Shift+wheel = sideways • Ctrl+wheel = zoom • double-click = reset")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Hospital Asset Tracker — Fullscreen Rollout Demo v11")
        ...
        self.hospital_view = ResponsiveMapView(self.hospital_scene, min_size=(420, 320), default_fit_mode="width")
        self.pilot_view = ResponsiveMapView(self.pilot_scene, min_size=(420, 180), default_fit_mode="whole")
        ...
        splitter.addWidget(
            MapCard(
                ...
                "Wheel scrolls top-to-bottom • Shift+wheel sideways • Ctrl+wheel zoom • fit-width default",
                self.hospital_view,
        ...
        splitter.addWidget(
            MapCard(
                ...
                "Wheel scrolls • Ctrl+wheel zoom • detailed pilot segment kept simple",
                self.pilot_view,
        ...
        if self.floorplan_path:
            self.statusBar().showMessage(
                f"  Loaded floorplan: {self.floorplan_path.name}. Pilot hallway and rollout paths are aligned to the floorplan corridor centerlines, with fullscreen-friendly map behavior."
            )
        else:
            self.statusBar().showMessage(
    ...
    def _reset_views(self):
        self.hospital_view.reset_to_default_fit(self.hospital_scene.reset_view_rect())
        self.pilot_view.reset_to_default_fit()
