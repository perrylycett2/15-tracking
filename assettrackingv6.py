...
SEGMENTS: Dict[str, SegmentSpec] = {
    # Long central pilot hallway aligned to the real corridor visible across the
    # lower-middle portion of the uploaded SJH Level 1 South floorplan.
    "PILOT": SegmentSpec(
        name="Pilot Hallway",
        path_points=(
            (160, 840),
    ...
    # Left-side vertical corridor in the west wing.
    "WEST_WING": SegmentSpec(
        name="West Wing Spine",
        path_points=((108, 225), (108, 365), (108, 530), (108, 700), (108, 835)),
        checkpoints={
            "NORTH": (108, 280),
            "MID": (108, 530),
            "SOUTH": (108, 785),
    ...
    # Top east horizontal hallway.
    "EAST_LOOP": SegmentSpec(
        name="East Upper Loop",
        path_points=((845, 177), (965, 177), (1090, 177), (1215, 180)),
        checkpoints={
            "WEST": (900, 177),
            "MID": (1060, 177),
            "EAST": (1188, 180),
    ...
    # Right-side main vertical hospital spine.
    "EAST_SPINE": SegmentSpec(
        name="East Main Spine",
        path_points=((1293, 105), (1293, 260), (1293, 430), (1293, 600), (1293, 820)),
        checkpoints={
            "NORTH": (1293, 180),
            "MID": (1293, 500),
            "SOUTH": (1293, 760),
    ...
    # Lower corridor in the east/south treatment area.
    "SOUTH_CLINICS": SegmentSpec(
        name="South Clinics Corridor",
        path_points=((930, 808), (1045, 808), (1175, 808), (1288, 808)),
        checkpoints={
            "WEST": (980, 808),
            "MID": (1120, 808),
            "EAST": (1245, 808),
...
class HospitalOverviewScene(QGraphicsScene):
    ...
    def _draw_background(self):
        ...
        if not self.loaded_pixmap.isNull():
            ...
            t = self.addText("Floorplan image not found.\nPlace the cropped PNG beside this script.")
    ...
    def _add_segment_shape(self, spec: SegmentSpec):
        points = [QPointF(x, y) for x, y in spec.path_points]
        path = points_to_path(points)

        outline_color = QColor(PALETTE["pilot_outline"] if spec.active else PALETTE["route_outline"])
        fill_color = QColor(78, 161, 255, 165) if spec.active else QColor(107, 114, 128, 80)
        width = 34 if spec.active else 24

        base_pen = QPen(outline_color, width, Qt.PenStyle.SolidLine if spec.active else Qt.PenStyle.DashLine)
        base_pen.setCapStyle(Qt.PenCapStyle.RoundCap)
        base_pen.setJoinStyle(Qt.PenJoinStyle.RoundJoin)
        item = self.addPath(path, base_pen)
        item.setOpacity(0.9 if spec.active else 0.75)
        item.setZValue(8 if spec.active else 4)

        glow_pen = QPen(fill_color, width - 8)
        glow_pen.setCapStyle(Qt.PenCapStyle.RoundCap)
        glow_pen.setJoinStyle(Qt.PenJoinStyle.RoundJoin)
        glow_item = self.addPath(path, glow_pen)
        glow_item.setZValue(9 if spec.active else 5)

        label = self.addText(("ACTIVE RFID PILOT\n" if spec.active else "ROLLOUT HALLWAY\n") + spec.name)
        label.setDefaultTextColor(QColor("#0B203A") if spec.active else QColor("#374151"))
        ...
        for checkpoint, point in spec.checkpoints.items():
            ...
            self.addEllipse(pt.x() - 8, pt.y() - 8, 16, 16,
                            ...
                            QBrush(QColor(PALETTE["sensor"]))).setZValue(16)
    ...
    def _draw_segment_overlay(self):
        ...
        legend = self.addText(
            "Aligned hospital rollout demo:\n"
            "• solid blue path = active RFID pilot hallway on a real corridor\n"
            "• dashed grey paths = additional rollout hallways on real corridors\n"
            "• select an asset in History to show its movement trail"
        )
        legend.setDefaultTextColor(QColor("#223142"))
        legend.setFont(QFont("Consolas", 9))
        legend.setPos(26, self.sceneRect().height() - 104)
        legend.setZValue(20)
    ...
    def show_history_trail(self, entries: List[Dict[str, str]]):
        ...
        pen = QPen(QColor(PALETTE["history"]), 6, Qt.PenStyle.DashLine)
        pen.setCapStyle(Qt.PenCapStyle.RoundCap)
        pen.setJoinStyle(Qt.PenJoinStyle.RoundJoin)
        trail = self.addPath(trail_path, pen)
        trail.setZValue(26)
        self.history_items.append(trail)

        for i, pt in enumerate(dedup_points, start=1):
            dot = self.addEllipse(pt.x() - 7, pt.y() - 7, 14, 14,
                                  QPen(QColor("#7C5B00"), 2),
...
class SidePanel(QWidget):
    ...
    def _build_ui(self):
        ...
        footer = QLabel("Double-click a map to auto-fit. Mouse wheel zooms.")
    ...
    def _build_demo_tab(self) -> QWidget:
        ...
        grp_demo = QGroupBox("RFID PILOT SCANS")
        ...
        grp_route = QGroupBox("ROUTE THROUGH HOSPITAL")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Hospital Asset Tracker — Fullscreen Rollout Demo v6")
        ...
        self.hospital_view = MapView(self.hospital_scene, min_size=(1080, 620))
        self.pilot_view = MapView(self.pilot_scene, min_size=(1080, 270))
        ...
        splitter.addWidget(self._wrap_view("Hospital-wide rollout aligned to real floorplan hallways", self.hospital_view))
        ...
        if self.floorplan_path:
            self.statusBar().showMessage(
                f"  Loaded floorplan: {self.floorplan_path.name}. Pilot hallway and rollout paths are aligned to corridor centerlines."
            )
        else:
            self.statusBar().showMessage(
    ...
    def _wrap_view(self, title: str, view: QGraphicsView) -> QWidget:
        ...
        hdr = QLabel(title)
        hdr.setStyleSheet(
            f"font-size:12px; font-weight:bold; color:{PALETTE['accent']}; font-family:Consolas;"
        )
        l.addWidget(hdr)
        l.addWidget(view, 1)
        return w
